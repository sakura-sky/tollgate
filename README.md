...
- **Prompt-cache token classes are counted, but not yet priced separately.** Every prompt-side token class is metered, so a cached request is no longer under-counted. All of them are charged at the model's base input rate, which does not match provider pricing: cache reads cost roughly a tenth of the input rate, while cache writes cost more than it (1.25x to 2x on Anthropic, by TTL). **A cache-read-heavy workload is therefore over-charged by up to about 10x on its prompt leg.** Concretely, a 200k-token cached prompt on a $3/1M input model is billed by the provider at roughly $0.06 and recorded by Tollgate at roughly $0.60. Size cache-heavy budgets accordingly until per-class rate columns land. A cache-write-heavy workload can still be under-charged, by at most half the write leg.
...
- **Text-only, single-tier usage is exact. Several other charges are not modelled.** A price is one input rate and one output rate per model, which cannot express: long-context tiering (several current models roughly double both rates above a 200k-token prompt, so an operator who sets base rates under-charges large requests by about 2x); per-modality rates (audio input bills several times text on some models); provider server-side tool fees such as web search or code execution, which are billed per request rather than per token; and grounding fees. All of these are under-charges. Model tiering and modality share the per-class rate work.
